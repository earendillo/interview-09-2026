...
describe('parseManifest', () => {
  it('returns one entry per remote, carrying its name', () => {
    ...
    expect(manifest[0].url).toBe('https://cdn.example/web/v2.0.0/remoteEntry.js');
  });
  ...
  it('keeps an explicit shareScope, which is how a remote opts out of shared React', () => {
    const isolated = {
      legacy: { url: 'https://cdn.example/legacy.js', contract: 1, shareScope: 'legacy' },
    };
...
describe('fallbackOf', () => {
  it('points the entry at the previous version, which is the rollback', () => {
    ...
    expect(rolledBack?.url).toBe('https://cdn.example/web/v1.9.3/remoteEntry.js');
