> **Historical.** This is the original brief the repository was started from,
> kept for provenance. It has drifted: the workspace now uses `@interview/*`
> rather than `@company/*`, and `apps/admin` became `apps/dashboard` alongside
> `apps/shell`, `apps/legacy` and `apps/api`. The README is the current
